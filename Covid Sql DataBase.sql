...
select 	location,
	...
	populations,
	total_cases,
	(total_cases/populations)*100 as PopulationPercentage
from dbo.coviddeaths
where location like '%netherlands%' and total_cases is not null
order by 1,2
...
select 	location,
	populations,
	Max(total_cases) as highestinfectioncount,max((total_cases/populations)*100) as PrecentofPopulationInfected
from dbo.coviddeaths
...
select 	continent,
	max(cast(total_deaths as int)) as HighestNumberOfDeath
from dbo.coviddeaths
...
--creating CTE to store data for later visualization
